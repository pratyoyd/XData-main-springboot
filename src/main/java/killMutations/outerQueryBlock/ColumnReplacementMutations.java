...
public class ColumnReplacementMutations {
	...
	public static ArrayList<Node> extractedNodes = new ArrayList<Node>();
	public static void extractedInnerCols(Node node)
	{
		
		if(node == null) return;		
		if(node.getAgg()!= null)extractedInnerCols(node.getAgg().getAggExp().getLeft());
		if(node.getAgg()!= null)extractedInnerCols(node.getAgg().getAggExp().getRight());		
	...
	public static void generateDataForkillingColumnReplacementMutationsInProjection(GenerateCVC1 cvc) throws Exception{
		try {
			...
			for (Node n: projectedColumns) {
				extractedInnerCols(n);
			}
			projectedColumns.addAll(extractedNodes);
			for(Node n: projectedColumns){
				...
				for(Map.Entry<String, Integer> e: cvc.getNoOfTuples().entrySet()){
					...
					for(int j = 0; j < requiredTable.getNoOfColumn(); j++){/**For each column of this table*/
						...
						if(col.getDataType() == candidate.getColumn().getDataType()){
							...
							if(replacement.getColumn().equals(candidate.getColumn())){
