...
public class GetSolverHeaderAndFooter {
	 ...
	 * Also, we are not consideringSystem.out.println(columnName+ " " + columnType+"-------------"); the input database. Hence there is no need for the input tuples and its range.
	 */
			...
			case 5: //DATE
			{
				...
				if(columnValue.size()>0){
					for(int j=0; j<columnValue.size(); j++){
						...
						long colValue1 = t.getTime()/86400000;
						long colValue = (long) Math.ceil(t.getTime()/86400000);
				...
				*/
				...
				cvc_datatype += "ISNULL_" + columnName +" : "+ columnName + " -> BOOLEAN;\n";
				*/
				cvc_datatype = "\n"+columnName+" : TYPE = SUBTYPE (LAMBDA (x: INT) : (x > "+(min-1)+" AND x < "+(max+1)+") );\n";
				HashMap<String, Integer> nullValuesInt = new HashMap<String, Integer>();
				for(int k=-99996;k>=-99999;k--){
					//isNullMembers += "ASSERT ISNULL_"+columnName+"("+k+");\n";
					//nullValuesInt.put(k+"",0);
