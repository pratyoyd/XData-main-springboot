...
public class GenerateConstraintsForConjunct {
	...
	public static String getConstraintsForConjuct(GenerateCVC1 cvc, QueryBlockDetails queryBlock, ConjunctQueryStructure conjunct) throws Exception {
		...
		//}			
				...
				//conObj.setRightConstraint(constraintGen.genPositiveCondsForPred(queryBlock, stringSelectionConds.get(k),l+offset-1));
				String strConstraints = conObj.getLeftConstraint()+" "+conObj.getOperator()+" "+conObj.getRightConstraint() ;
				...
				for(String stringConstriants :solvedStringConstraint) {
					constraintString += stringConstriants ;
				}
		...
		if(constrList != null && constrList.size()>0){
			for(ConstraintObject conObj : constrList) {
					String constraint = "ASSERT("+conObj.getLeftConstraint()+" "+conObj.getOperator()+" "+conObj.getRightConstraint()+") ;\n" ;
	...
	public static Constraints getConstraintsInConjuct(GenerateCVC1 cvc, QueryBlockDetails queryBlock, ConjunctQueryStructure conjunct) throws Exception {
		...
		Vector<Node> selectionConds = conjunct.getSelectionConds();
		//selectionConds.addAll(conjunct.allConds);
		for(int k=0; k< selectionConds.size(); k++){
